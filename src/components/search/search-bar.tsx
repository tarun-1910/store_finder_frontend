...
const SUGGESTIONS = [    
  "Sarees",  "Furniture", "Jewelry"
];

export function SearchBar({ defaultQuery = "", large = false }: { defaultQuery?: string; large?: boolean }) {
  ...
  return (
    <div className={large ? "w-full max-w-2xl mx-auto" : "w-full"}>
      <form onSubmit={handleSearch} className="flex gap-2">
        ...
        <Button type="submit" size={large ? "lg" : "default"} className="bg-emerald-600 hover:bg-emerald-700">
          Search
        </Button>
      </form>
      {large && (
